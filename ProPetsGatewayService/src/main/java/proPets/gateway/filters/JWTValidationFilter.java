...
public class JWTValidationFilter implements Filter {
		...
		if (!checkPointCut(path, method) && !checkStartPathAdditional(path) && checkStartPath(path)) {
			if (auth.startsWith("Bearer")) {
				...
				try {
					...
					System.out.println("Im adding principal");
	...
	private boolean checkStartPath(String path) {
		...
		check = check || path.startsWith("/lostFound/found/v1/post") || path.startsWith("/message/v1/post");
		return check;
	}
	
	// might be false
	private boolean checkStartPathAdditional(String path) {
		boolean check = path.startsWith("/account") || path.startsWith("/send") || path.startsWith("/search") || path.startsWith("/convert") || path.startsWith("/lostFound/lost/v1/all_matched") || path.startsWith("/lostFound/lost/v1/new_matched")|| path.startsWith("lost/v1/accessCode");
